...
## 12-3 · Landing page — BUILT 2026-08-14

**Status: built and verified locally; not deployed** (hosting is still step 2 of the release
sequence). What shipped, and where it diverged from the plan below:

- `/` is the landing page on web; the app home moved to `/app` on **both** platforms
  (`src/app/app.tsx`), with `src/app/index.tsx` redirecting on native. All 13 `router` calls to
  `'/'` were retargeted, and `typedRoutes` type-checks them.
- The page is **plain semantic DOM, not react-native-web** — `Text` renders as a `<div>`, and
  `useTheme()` reads `useSettingsStore`, which would have kept the content out of the static
  HTML. It lives in `src/components/landing/`, **not** under `src/app/`: every file in the app
  directory becomes a route (which is why `dist/editStyles.html` exists — a pre-existing stray
  route worth cleaning up separately).
- Not `index.web.tsx`. Expo Router's per-platform `require.context` globs do not clearly treat
  a `.web` suffix in the app directory as a platform variant, so the split is done one level
  down via `LandingPage.web.tsx` / `LandingPage.tsx`, the pattern `TopBar` already uses.
- `expo-router/head` **does** work under `output: "static"`, despite being absent from the SDK 55
  API reference. Verified: exactly one `<title>`, plus description, canonical, OG and Twitter
  tags in `dist/index.html`.
- **Static-render acceptance test passes**: `dist/index.html` went from ~10 characters of
  visible text to 5,084, with 1 `<h1>`, 6 `<h2>` and 6 `<h3>`, and the FAQ answers present as
  text (`<details open>`).
- Hero images are generated by `scripts/build-hero-images.py` (Python/Pillow — the repo has no
  JS image toolchain). WebP + JPEG at 960/1440/1920; quality falls as width rises because the
  large variants only reach 2x displays and most of the photo sits under an 88% scrim.
  841 KB source → 88 KB at 1440, 226 KB at 1920.
- **The font gate is not a problem.** Measured on warm and cold cache: no blank after
  hydration. The 12 local font files resolve at ~55 ms, well before the 3.6 MB entry bundle
  parses and hydrates, so `useFonts` is already true at first client render. `_layout.tsx` was
  left alone.
- **Known cost, not yet addressed:** `dist/index.html` loads the whole app bundle — 3.6 MB raw,
  936 KB gzipped — to render a marketing page. It does not delay LCP (markup is prerendered,
  CSS is inline, the hero is its own request) but it does delay the demo becoming clickable.
  Code-splitting the landing route is the obvious follow-up.
- Demo: verified it calls the real pipeline and keeps tfjs/model out of the initial load
  (**zero** such requests until the play button is clicked). **Not yet verified against real
  audio — `public/demo/sample.wav` does not exist.**

Original plan follows.

### 12-3 (original) · Landing page — a surface that does not exist yet
