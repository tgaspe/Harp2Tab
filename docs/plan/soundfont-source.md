...
**The file is not committed.** The asset directory it generates is
(`public/soundfonts/musescore-general-0.2.0/`). Re-download from the URL above and check the
SHA-256 before re-running the converter — the mirror path has no version in it, so the
checksum is the only thing actually pinning this.
