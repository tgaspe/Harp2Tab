...
| `spike-osmd.ts` | 20 | The renderer gate (Phase 18): OpenSheetMusicDisplay engraves the MusicXML this repo generates, headless under jsdom — notehead→`TabNote.id` mapping, self-contained SVG, 500-note cost — and that `osmd.web.ts`, the wrapper the app actually calls, paginates to A4 and highlights reversibly. jsdom has no layout engine, so it proves the pipeline and not the picture |
...
| `verify-playback.ts` | 17 | The web scheduler (11-6): MIDI channel allocation (percussion never takes a melodic channel), note-on/note-off event building across a seek, a dense 3,840-note track, and that repeated restarts don't exhaust the browser's AudioContext budget |
