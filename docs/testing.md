...
| `spike-osmd.ts` | 13 | The renderer gate (Phase 18): OpenSheetMusicDisplay engraves the MusicXML this repo generates, headless under jsdom — notehead→`TabNote.id` mapping, self-contained SVG, 500-note cost. jsdom has no layout engine, so it proves the pipeline and not the picture |
